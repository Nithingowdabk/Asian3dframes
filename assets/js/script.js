	...
	function setupProductMockupStudio() {
		...
		const drawBacklightGlow = (rect, frameType) => {
			...
			const warmBloom = ctx.createRadialGradient(cx, cy, innerR, cx, cy, outerR);
			   warmBloom.addColorStop(0, "rgba(255, 248, 225, 0.10)");
			   warmBloom.addColorStop(0.42, "rgba(248, 220, 165, 0.045)");
			   warmBloom.addColorStop(0.78, "rgba(236, 196, 120, 0.018)");
			...
			const topAura = ctx.createLinearGradient(rect.x, rect.y, rect.x, rect.y + rect.h);
			   topAura.addColorStop(0, "rgba(255, 251, 239, 0.035)");
		...
		const drawPhotoGloss = (rect, frameType) => {
			...
			const gloss = ctx.createLinearGradient(rect.x, rect.y, rect.x, rect.y + rect.h);
			   gloss.addColorStop(0, "rgba(255,255,255,.03)");
			   gloss.addColorStop(0.28, "rgba(255,255,255,.012)");
			   gloss.addColorStop(0.55, "rgba(255,255,255,.003)");
			...
			const sweep = ctx.createLinearGradient(rect.x, rect.y, rect.x + rect.w, rect.y + rect.h);
			   sweep.addColorStop(0, "rgba(255,255,255,.018)");
			   sweep.addColorStop(0.22, "rgba(255,255,255,.006)");
		...
		const drawProductPop = (rect, frameType) => {
			if (frameType !== "mobile") return;
			ctx.save();
			   ctx.strokeStyle = "rgba(255, 246, 214, 0.09)";
			   ctx.lineWidth = Math.max(4, rect.w * 0.014);
			   ctx.shadowColor = "rgba(255, 228, 164, 0.03)";
			...
			const rimGlow = ctx.createLinearGradient(rect.x, rect.y, rect.x, rect.y + rect.h);
			   rimGlow.addColorStop(0, "rgba(255,255,255,0.02)");
			   rimGlow.addColorStop(0.26, "rgba(255,255,255,0.008)");
			   rimGlow.addColorStop(0.55, "rgba(255,244,214,0.003)");
			...
			const centerBoost = ctx.createRadialGradient(
			   ...
			   centerBoost.addColorStop(0, "rgba(255, 246, 224, 0.012)");
			   centerBoost.addColorStop(0.65, "rgba(255, 246, 224, 0.004)");
